...
public class Dates {
    ...
    public static String dataBrComBarraParaDataAmericanaComTraco(String date) {
        if (date != null) {
            date = date.replace("/", "");
            try {
    ...
    public static Date dataComTracoParaDataComBarra(String date) {
        if (date != null) {
            date = date.replace("-", "/");
            try {
    ...
    public static String[] ArrayDataHoje() {
        ...
        result[0] = Dates.data().substring(0, 2);
        result[1] = Dates.data().substring(3, 5);
        result[2] = Dates.data().substring(6, 10);
        return result;
    }
    ...
    public static int[] ArrayDataHojeInt() {
        ...
        result[0] = Integer.parseInt(Dates.data().substring(0, 2));
        result[1] = Integer.parseInt(Dates.data().substring(3, 5));
        result[2] = Integer.parseInt(Dates.data().substring(6, 10));
        return result;
    }
    ...
    public static long calculoDosDias(Date dInicial, Date dFinal) {
        Dates.converteDataParaInteger(Dates.converteData(dInicial));
        Dates.converteDataParaInteger(Dates.converteData(dFinal));
        if (((dInicial != null) && (dFinal != null))
                && (Dates.converteDataParaInteger(Dates.converteData(dInicial)) < Dates.converteDataParaInteger(Dates.converteData(dFinal)))) {
    ...
    public String incrementarMeses(int qtd, String data) {
        if (isDataValida(data)) {
            try {
                int c = 0;
                int[] d = Dates.DataToArrayInt(data);
                if ((d[1] + qtd) > 12) {
                    int tmp = (d[1] + qtd);
                    while (tmp > 12) {
                        tmp -= 12;
    ...
    public String decrementarDias(int qtd, String data) {
        ...
        calendar.setTime(Dates.converte(data));
    ...
    public String incrementarMesesUltimoDia(int qtd, String data) {
        if (isDataValida(data)) {
            try {
                int c = 0;
                int[] d = Dates.DataToArrayInt("32/" + data.substring(3, 10));
                if ((d[1] + qtd) > 12) {
                    int tmp = (d[1] + qtd);
                    while (tmp > 12) {
                        tmp -= 12;
    ...
    public String mascararData(String data) {
        return Dates.converteData(Dates.converte(data));
    }
    ...
    public static int converteDataParaInteger(String data) {
        if (isDataValida(data)) {
            return Integer.parseInt(Dates.DataToArrayString(data)[2] + Dates.DataToArrayString(data)[1] + Dates.DataToArrayString(data)[0]);
        }
        return 0;
    }
...
//                    + dif.get(GregorianCalendar.DAY_OF_MONTH) + " dia(s)");
    ...
    public boolean faixaCincoAnosApos(String data) {
        int dataHoje = Dates.converteDataParaRefInteger(Dates.data());
        int dataAntes = Dates.converteDataParaRefInteger(decrementarMeses(60, Dates.data()));
        int dataM = Dates.converteDataParaRefInteger(data);
    ...
    public int calcularIdade(String dataNascimento) {
        ...
        if (isDataValida(dataNascimento)) {
            ...
            String dataHoje = Dates.data();
            int[] dataH = Dates.DataToArrayInt(dataHoje);
            int[] dataN = Dates.DataToArrayInt(dataNascimento);
            idade = dataH[2] - dataN[2];
            int[] novaData = Dates.DataToArrayInt(incrementarAnos(idade, dataNascimento));
    ...
    public int calcularIdade(Date data) {
        ...
        return calcularIdade(Dates.converteData(data));
    }
...
//                }
    ...
    public static Boolean between(String date, String start, String finish) {
        try {
            int d = Dates.converteDataParaInteger(date);
            int s = Dates.converteDataParaInteger(start);
            int f = Dates.converteDataParaInteger(finish);
            if (d >= s && d <= f) {
                return true;
            }
        } catch (Exception e) {
    ...
    /**
     * Converte refrência para data
     *
     * @param reference
     * @return
     */
    public static Date convertReferenceToDate(String reference) {
        return Dates.converte("01/" + reference);
    }

    /**
     * Converte referência para integer
     *
     * @param reference
     * @return
     */
    public static Integer convertReferenceToInteger(String reference) {
        try {
            reference = reference.replace("/", "");
            return Integer.parseInt(reference.substring(2, 6) + reference.substring(0, 2));
...
//            int hours = minutes / 60;
    ...
    public static Integer diffMonths(Date startDate, Date endDate) {
        return diffMonths(Dates.converteData(startDate), Dates.converteData(endDate));
    }

    public static Integer diffMonths(String startDate, String endDate) {
        try {
            Calendar startCalendar = new GregorianCalendar();
            startCalendar.setTime(Dates.converte(startDate));
            Calendar endCalendar = new GregorianCalendar();
            endCalendar.setTime(Dates.converte(endDate));
    ...
    public Date addDays(Integer days) {
        Dates dh = new Dates();
        if (days >= 0) {
            return Dates.converte(dh.incrementarDias(days, Dates.data()));
        }
        return new Date();
    }
    ...
    public Date removeDays(Integer days) {
        ...
        return Dates.converte(dh.decrementarDias(days, Dates.data()));
    }
